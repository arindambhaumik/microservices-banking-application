...
public class AccountDetailsControllerImpl implements AccountDetailsController {
	...
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Successfully fetched the accounts."),
	...
	/**
	 * Returns all transactions for a account number
	 * 
	...
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Successfully fetched the transactions."),
			@ApiResponse(code = 401, message = "transaction retrieval failed."),
			@ApiResponse(code = 500, message = "Something went wrong. Internal server error.") })
	@GetMapping(path = RestfulEndPoints.TRANSACTION_RETRIEVE)
	public List<Transaction> retrieveTransactions(@PathVariable(name = "accountID") int accountID) {
		return accountDetailsService.retrieveTransactions(accountID);
	}
}
